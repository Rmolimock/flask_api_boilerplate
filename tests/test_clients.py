...
def test_client_method(mocker, is_valid_id, method_no_post, make_request, mock_obj_if_valid_id):
    # MOCK VALUES
    client_class_in_route = mocker.patch("routes.client.client.Client")
    method = method_no_post

    # SETUP
    valid_client_in_route = mock_obj_if_valid_id(client_class_in_route)
    client_id = valid_client_in_route.id if is_valid_id else str(uuid4())
